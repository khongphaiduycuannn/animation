...
public class CropView extends AppCompatImageView {
    ...
    private void drawOverlay(Canvas canvas) {
        mPaintTrans.setAntiAlias(true);
        mPaintTrans.setFilterBitmap(true);
        mPaintTrans.setColor(mOverlayColor);
        ...
        if (!mIsAnimating && (mCropMode == CropMode.CIRCLE || mCropMode == CropMode.CIRCLE_SQUARE)) {
            ...
            canvas.drawPath(path, mPaintTrans);
        } else {
            path.addRect(overlayRect, Path.Direction.CW);
    ...
    public void updateRectF(RectF frameRect, RectF imgRect) {
        this.mFrameRect = frameRect;
        this.mImageRect = imgRect;
        invalidate();
    }

    public void setCropRect(RectF rect) {
        if (rect == null) return;
        mFrameRect = new RectF(rect);
        invalidate();
    }
    ...
    public Rect getCropRect() {
        Bitmap source = getBitmap();
        if (source == null) return null;
        return calcCropRect(source.getWidth(), source.getHeight());
    }

    public static Bitmap getBitmapCropByRect(Rect cropRect, Bitmap srcBitmap) {
        if (srcBitmap == null) return null;
        if (srcBitmap.isRecycled()) return null;
        Bitmap result = Bitmap.createBitmap(srcBitmap, cropRect.left, cropRect.top, cropRect.width(),
                cropRect.height(), null, false);
        srcBitmap.recycle();
        return result;
    }
